...
class RustArbitrageEngine:
    ...
    async def validate_arbitrage_path(
        self, path: ArbitragePath, current_prices: Dict[str, float], gas_price_gwei: float
    ) -> Dict[str, Any]:
        """
    ...
    async def _execute_rust_engine(self, input_data: Dict[str, Any], operation: str) -> List[Any]:
        """
        ...
        try:
            ...
            try:
                stdout, stderr = await asyncio.wait_for(
                    process.communicate(), timeout=self.config.max_execution_time_seconds
                )
...
class RustEngineManager:
    ...
    def get_aggregate_metrics(self) -> Dict[str, Any]:
        ...
        avg_search_time = sum(
            engine.average_search_time_ms * engine.total_searches for engine in self.engines
        ) / max(total_searches, 1)

        avg_cache_hit_rate = sum(engine.cache_hit_rate for engine in self.engines) / len(
            self.engines
        )
