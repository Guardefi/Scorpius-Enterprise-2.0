...
class ChainRegistry:
                    ...
                    with open(file_path, "r") as f:
                        ...
                        if chain_id_str:
                            ...
                            if "name" not in chain_config:
                                chain_config["name"] = (
                                    filename.replace(".json", "").replace("chain_", "").capitalize()
                                )
    ...
    def get_web3_provider(self, chain_id: int) -> Optional[Web3]:
        ...
        try:
            ...
            logger.error(
                f"Error creating Web3 provider for chain {chain_id} ({rpc_url}): {e}", exc_info=True
            )
            return None
...
class MultiChainMonitor:
    ...
    async def remove_chain_monitor(self, chain_id: int) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Exception while stopping monitor for chain {chain_id}: {e}", exc_info=True
            )
            return False
