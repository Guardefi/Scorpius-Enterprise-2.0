...
class MEVEngine:
    ...
    def __init__(self, config: Config):
        ...
        self._bundle_queue: asyncio.Queue[tuple[BundleRequest, MEVOpportunity]] = asyncio.Queue(
            maxsize=1000
        )
        ...
        while self._running:
            try:
                ...
                if self._tx_processing_times:
                    self.metrics.avg_tx_processing_time_ms = sum(self._tx_processing_times) / len(
                        self._tx_processing_times
                    )

                if self._bundle_submit_times:
                    self.metrics.avg_bundle_submit_time_ms = sum(self._bundle_submit_times) / len(
