...
class AbstractStrategy(ABC):
    ...
    def __init__(self, strategy_type: StrategyType, config: StrategyConfig, explain: bool = False):
        """
        Initialize strategy
        ...
        if explain:
            ...
            handler.setFormatter(
                logging.Formatter(f"[{strategy_type.value.upper()}] %(asctime)s - %(message)s")
            )
    ...
    async def _process_opportunities(self) -> None:
        ...
        while self._running:
            try:
                ...
                opportunity = await asyncio.wait_for(self._opportunities.get(), timeout=1.0)
...
class StrategyRegistry:
    ...
    def register_strategy(
        self, name: str, strategy: AbstractStrategy, file_path: Optional[str] = None
    ) -> None:
    ...
    def get_enabled_strategies(self) -> Dict[str, AbstractStrategy]:
        ...
        return {
            name: strategy for name, strategy in self._strategies.items() if strategy.is_enabled()
        }
