...
class RelayClient(ABC):
    ...
    @abstractmethod
    async def submit_bundle(self, bundle: BundleRequest, bundle_id: str) -> BundleSubmissionResult:
        """
...
class FlashbotsRelay(RelayClient):
    ...
    async def submit_bundle(self, bundle: BundleRequest, bundle_id: str) -> BundleSubmissionResult:
        ...
        try:
            ...
            async with self.session.post(self.endpoint.url, data=body, headers=headers) as response:
            ...
            body = json.dumps(
                {
                    ...
                    "params": [{"bundleHash": bundle_id, "blockNumber": hex(bundle.block_number)}],
                }
            )
    ...
    async def _build_flashbots_bundle(self, bundle: BundleRequest) -> Dict[str, Any]:
        ...
        flashbots_bundle = {"txs": transactions, "blockNumber": hex(bundle.block_number)}
...
class BundleSubmitter:
    ...
    async def submit_bundle(
        self, bundle: BundleRequest, bundle_id: str, preferred_relays: Optional[List[str]] = None
