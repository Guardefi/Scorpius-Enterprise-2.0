...
class ExecutionEngine:
    ...
    def __init__(self, web3: Web3, private_key: str, wallet_address: str, explain: bool = False):
        """
    ...
    async def execute_opportunity(
        ...
        try:
            if self.explain:
                self.logger.info(
                    f"Executing opportunity {opportunity.id} " f"(execution: {execution_id[:8]}...)"
                )
            ...
            if execution.expected_profit_wei < self.profit_threshold_wei:
                ...
                execution.error = (
                    f"Profit below threshold: {execution.expected_profit_wei/10**18:.6f} ETH"
                )
                return execution
    ...
    async def _reserve_bundle_nonces(self, bundle: BundleRequest, execution_id: str) -> List[Any]:
        """
            ...
            try:
                ...
                for execution_id, execution in self.active_executions.items():
                    ...
                    if (
                        ...
                        not in [ExecutionStatus.INCLUDED, ExecutionStatus.FAILED]
                    ):
                        ...
                        if included_results:
                            ...
                            if execution.net_profit_wei:
                                ...
                                self.metrics["best_profit_wei"] = max(
                                    self.metrics["best_profit_wei"], execution.net_profit_wei
                                )
