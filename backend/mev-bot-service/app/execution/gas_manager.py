...
class GasManager:
    ...
    async def optimize_for_mev_profit(
        self, tx_params: TxParams, expected_profit_wei: int, max_gas_cost_ratio: float = 0.3
    ) -> GasEstimate:
        """
    ...
    async def _calculate_optimal_gas_prices(
        self, current_gas: GasPriceData, strategy: GasStrategy, max_profit_wei: Optional[int] = None
    ) -> Dict[str, int]:
        """
        Calculate optimal gas prices based on strategy
