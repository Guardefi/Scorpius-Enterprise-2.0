...
class FlashLoanArbitrageExecutor:
    ...
    def __init__(self, config):
        ...
        self.aggregator_contract = self.w3.eth.contract(
            address=self.aggregator_contract_address, abi=self.abis["flash_loan_aggregator"]
        )
        self.logger.info(
    ...
    def _encode_aggregator_call(
        self, provider_id: int, loan_token: str, loan_amount: int, strategy_sequence: list
    ) -> str:
        """
        ...
        for action in strategy_sequence:
            if action.get("type") == "swap":
                ...
                encoded_action = (Web3.toChecksumAddress(action["target"]), action["data"])
            else:
        ...
        return self.aggregator_contract.encodeABI(
            ...
            args=[provider_id, Web3.toChecksumAddress(loan_token), loan_amount, action_struct],
        )
    ...
    async def execute_arbitrage(self, opportunity: dict):
        ...
        try:
            ...
            if receipt and receipt.status == 1:
                ...
                fail_reason = (
                    f"Transaction reverted or status is 0."
                    if receipt
