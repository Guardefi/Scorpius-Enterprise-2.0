        ...
        async with self._history_lock:
            try:
                ...
                for block_num in range(start_block, latest_block_num + 1):
                    fetch_tasks.append(
                        asyncio.to_thread(self.w3.eth.get_block, block_num, full_transactions=True)
                    )
    ...
    def _calculate_priority_fee_target(self) -> int:
        ...
        try:
            ...
            target_priority_fee = max(
                self.MIN_PRIORITY_FEE_WEI, min(calculated_fee, self.MAX_PRIORITY_FEE_WEI)
            )
            self.logger.debug(
            ...
            async with self._history_lock:
                ...
                if max_fee_per_gas > max_allowed_gas:
                    ...
                    target_priority_fee = min(
                        target_priority_fee,
                        max(self.MIN_PRIORITY_FEE_WEI, max_fee_per_gas - predicted_base_fee),
                    )
            result = {"maxFeePerGas": max_fee_per_gas, "maxPriorityFeePerGas": target_priority_fee}
            self.logger.info(
