...
class UltimateMEVBot:
    ...
    def get_bot_status(self) -> MEVBotStatus:
        ...
        total_profit = sum(strategy.stats.total_profit for strategy in self.strategies.values())
...
async def toggle_strategy(request: StrategyToggleRequest):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to toggle strategy {request.strategy_type}"
            )
...
async def get_performance_metrics():
    ...
    if total_executions > 0:
        successful_executions = sum(
            strategy.stats.successful_executions for strategy in mev_bot.strategies.values()
        )
...
if __name__ == "__main__":
    ...
    uvicorn.run("mev_api_server:app", host="0.0.0.0", port=port, log_level="info", reload=False)
