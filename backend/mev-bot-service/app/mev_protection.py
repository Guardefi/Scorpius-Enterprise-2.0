...
class MEVMetrics:
    ...
    def update_provider_metrics(self, provider: str, success: bool, latency: float, gas_used: int):
        ...
        data["avg_latency"] = ((data["avg_latency"] * (data["submissions"] - 1)) + latency) / data[
            "submissions"
        ]
        if success:
...
class MEVProtectionManager:
    ...
    def __init__(self, config, w3: Web3):
        ...
        self.providers = {
            ...
            "eden": EdenRPC(endpoint=config["EDEN_ENDPOINT"], api_key=config["EDEN_API_KEY"]),
            "bloxroute": CloudApi(
                project_id=config["BLOXROUTE_PROJECT_ID"], secret_key=config["BLOXROUTE_SECRET"]
            ),
        }
...
class EnhancedMEVProtection:
    ...
    def _parse_revert_reason(self, error: Exception) -> str:
        ...
        try:
            ...
            decoded = self.mev_manager.providers["flashbots"].contract.decode_function_input(
                "0x" + hex_data
            )
