...
class FlashLoanArbitrageBot:
    ...
    async def _opportunity_processor(self):
        ...
        while self.running or not self.opportunity_queue.empty():
            ...
            try:
                ...
                if sim_results.get("success") and sim_results.get("profit", 0) > self.config.get(
                    "MIN_PROFIT_THRESHOLD", 0.1
                ):
                    ...
                    if prediction < self.config.get("MIN_PREDICTION_THRESHOLD", 0.7):
                        ...
                        self.opportunity_queue.task_done()
                        continue
                    execution_success, execution_details = await self._execute_arbitrage(
                        opportunity, sim_results
                    )
                    ...
                    if execution_success:
                        ...
                        await self.alert_system.trigger_alert(
                            "ARB_FAILURE", {"opportunity": opportunity, "reason": execution_details}
                        )
                    if self.config.get("RL_ENABLED"):
                        try:
                            tx_hash_executed = (
                                execution_details.get("tx_hash") if execution_success else None
                            )
                            profit_achieved = (
                                execution_details.get("profit", 0) if execution_success else 0
                            )
                            ...
                            self.logger.error(
                                f"Error updating ML/RL model: {ml_update_e}", exc_info=True
                            )
                else:
                    self.logger.info(
                ...
                self.stats["errors"] += 1
                self.logger.error(
                    f"Error processing opportunity {tx_hash[:10]}...: {e}", exc_info=True
                )
            finally:
                self.opportunity_queue.task_done()
    ...
    def _prepare_simulation_strategy(self, opportunity):
        ...
        params = {
            ...
            "target_contracts": (
                [opportunity.get("contract_address")] if opportunity.get("contract_address") else []
            ),
            "flashloan_provider": self.config.get("DEFAULT_FLASHLOAN_PROVIDER", "aave"),
        }
        return strategy, params
    ...
    def get_stats(self):
        ...
        ops_per_hour = (
            (self.stats["opportunities_detected"] / run_time * 3600) if run_time > 0 else 0
        )
        success_rate = (
            (self.stats["successful_transactions"] / self.stats["transactions_executed"] * 100)
            if self.stats["transactions_executed"] > 0
            else 0
        )
        avg_profit = (
            (self.stats["total_profit_gross_eth"] / self.stats["successful_transactions"])
