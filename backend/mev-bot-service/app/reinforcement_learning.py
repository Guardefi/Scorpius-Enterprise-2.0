...
class RLTrainingSystem:
    ...
    def __init__(self, config, rl_model, optimizer):
        ...
        # Metrics tracking
        self.metrics = {"episodes": 0, "total_reward": 0.0, "rewards_history": deque(maxlen=100)}
    ...
    def save_model(self):
        ...
        with open(metrics_path, "w") as f:
            json.dump(
                {k: list(v) if isinstance(v, deque) else v for k, v in self.metrics.items()}, f
            )
...
class ZKModelProver:
    ...
    async def _run_snarkjs(self, cmd: list) -> dict:
        ...
        proc = await asyncio.create_subprocess_exec(
            "snarkjs", *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
