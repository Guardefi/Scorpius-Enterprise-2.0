...
from arbitrage_math import (
    ...
    get_dex_config,
    simulate_swap_step,
    estimate_gas_cost_in_base_token,
    get_eth_price,
    Token,
)
from abis import IUiPoolDataProviderV3_ABI, IPoolAddressesProvider_ABI, IPool_ABI, IPriceOracle_ABI
...
class LiquidationMonitor:
    ...
    def __init__(self, provider):
        ...
        self.aave_config = {
            ...
            "MONITORED_PAIRS": [
                {"collateral": p.get("collateral", "").upper(), "debt": p.get("debt", "").upper()}
        ...
        try:
            ...
            self.aaveV3["poolAddress"] = Web3.to_checksum_address(
                await with_retry_async(
                    lambda: addresses_provider.functions.getPool().call(), "Aave getPool"
                )
            )
        ...
        for user_addr in risky_users:
            tasks.append(
                self._check_single_user(
                    user_addr, self.aave_config, eth_price, reserve_prices_vs_eth, semaphore
                )
            )
