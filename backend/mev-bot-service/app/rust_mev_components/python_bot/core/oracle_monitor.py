        ...
        try:
            if oracle_type == "CHAINLINK":
                ...
                for asset_symbol in self.config.get("monitoredAssets", []):
                    ...
                    if (
                        ...
                        try:
                            ...
                            logger.error(f"Failed init Chainlink feed for {asset_symbol}: {e}")
                ...
                self.aave_oracle_contract = self.w3.eth.contract(
                    address=Web3.to_checksum_address(oracle_address), abi=IAaveOracle_ABI
                )
                ...
                try:
                    ...
                    if oracle_type == "CHAINLINK":
                        ...
                        if token:
                            ...
                            price_scaled_usd = (price_vs_eth * eth_price_data["price"]) // (
                                10**18
                    ...
                    logger.warning(f"Failed fetching on-chain price for {asset_symbol}: {e}")
