...
class MempoolScanner:
    ...
    def __init__(
        ...
        self.backpressure = BackPressureManager(
            max_queue_size=queue_size, threshold=0.8  # Start backpressure at 80% capacity
        )
        ...
        while self._running:
            try:
                ...
                async with websockets.connect(
                    network_config.ws_url, ping_interval=20, ping_timeout=10, close_timeout=10
                ) as websocket:
        ...
        while self._running:
            try:
                ...
                if queue_utilization > 80:
                    self.logger.warning(f"High queue utilization: {queue_utilization:.1f}%")
