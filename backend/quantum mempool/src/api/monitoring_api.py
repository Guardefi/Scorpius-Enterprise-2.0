...
class MonitoringAPI:
    ...
    def _setup_routes(self):
        ...
        async def get_system_metrics(
            start_time: datetime = Query(default=None, description="Start time for metrics"),
            end_time: datetime = Query(default=None, description="End time for metrics"),
            metric_names: List[str] = Query(default=[], description="Specific metrics to retrieve")
        ):
            ...
            try:
                ...
                # Collect metrics
                metrics = await self._collect_metrics(start_time, end_time, metric_names)
        ...
        async def get_performance_report(
            duration_hours: int = Query(default=24, description="Report duration in hours")
        ):
            ...
            try:
                ...
                memory_metrics = await self._collect_memory_metrics(start_time, end_time)
                network_metrics = await self._collect_network_metrics(start_time, end_time)
                threat_metrics = await self._collect_threat_detection_metrics(start_time, end_time)
    ...
    async def _collect_metrics(
        ...
        if not metric_names:
            metric_names = ["cpu_usage", "memory_usage", "network_throughput", "threat_detection_rate"]
        
        for metric_name in metric_names:
            data_points = await self._generate_metric_data(metric_name, start_time, end_time)
            
            all_metrics.append(MetricSeries(
    ...
    async def _collect_cpu_metrics(self, start_time: datetime, end_time: datetime) -> MetricSeries:
        """Collect CPU performance metrics."""
        data_points = await self._generate_metric_data("cpu_usage", start_time, end_time)
        return MetricSeries(
    ...
    async def _collect_memory_metrics(self, start_time: datetime, end_time: datetime) -> MetricSeries:
        """Collect memory usage metrics."""
        data_points = await self._generate_metric_data("memory_usage", start_time, end_time)
        return MetricSeries(
    ...
    async def _collect_network_metrics(self, start_time: datetime, end_time: datetime) -> MetricSeries:
        """Collect network throughput metrics."""
        data_points = await self._generate_metric_data("network_throughput", start_time, end_time)
        return MetricSeries(
    ...
    async def _collect_threat_detection_metrics(self, start_time: datetime, end_time: datetime) -> MetricSeries:
        """Collect threat detection performance metrics."""
        data_points = await self._generate_metric_data("threat_detection_rate", start_time, end_time)
        return MetricSeries(
