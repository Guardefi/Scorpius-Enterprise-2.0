...
class WebSocketAPI:
    ...
    async def start_server(self):
        ...
        try:
            ...
            self.logger.info(f"WebSocket server started on {self.config.websocket_config.host}:{self.config.websocket_config.port}")
    ...
    async def subscribe_client(self, websocket: WebSocketServerProtocol, channels: list):
        """Subscribe client to channels."""
        valid_channels = {
            'alerts', 'metrics', 'system_status', 
            'mempool_stats', 'quantum_threats'
        }
    ...
    async def unsubscribe_client(self, websocket: WebSocketServerProtocol, channels: list):
    ...
    async def broadcast_alerts(self):
        ...
        while self.is_running:
            try:
                ...
                await self.broadcast_to_channel('quantum_threats', {
                    'type': 'quantum_threat',
                    'data': alert_data
                })
    ...
    async def broadcast_metrics(self):
        ...
        while self.is_running:
            try:
                metrics_data = await asyncio.wait_for(self.metrics_queue.get(), timeout=1.0)
                await self.broadcast_to_channel('metrics', {
                    'type': 'metrics',
                    'data': metrics_data
                })
                await self.broadcast_to_channel('mempool_stats', {
                    'type': 'mempool_stats',
                    'data': metrics_data
                })
            except asyncio.TimeoutError:
    ...
    async def broadcast_system_status(self):
        ...
        while self.is_running:
            try:
                system_data = await asyncio.wait_for(self.system_queue.get(), timeout=1.0)
                await self.broadcast_to_channel('system_status', {
                    'type': 'system_status',
                    'data': system_data
                })
            except asyncio.TimeoutError:
