...
from sqlalchemy import (
    ...
    ForeignKey, Index, CheckConstraint, DECIMAL, JSON
)
...
class QuantumSignature(Base, TimestampMixin, AuditMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint('temporal_clustering >= 0 AND temporal_clustering <= 1', name='temporal_range'),
        CheckConstraint('fee_uniformity >= 0 AND fee_uniformity <= 1', name='fee_range'),
    )
...
class ThreatAlert(Base, TimestampMixin, AuditMixin):
    ...
    __table_args__ = (
        ...
        Index('idx_alert_compliance', 'incident_classification', 'regulatory_notification_required'),
        CheckConstraint('confidence_score >= 0 AND confidence_score <= 1', name='alert_confidence_range'),
    )
...
class AuditLog(Base, TimestampMixin):
    ...
    blockchain_hash = Column(String(64), nullable=True, unique=True)  # For blockchain audit trail
...
class ComplianceReport(Base, TimestampMixin, AuditMixin):
    ...
    framework = Column(String(50), nullable=False, index=True)  # SOX, GDPR, PCI_DSS, etc.
...
class IncidentReport(Base, TimestampMixin, AuditMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint('escalation_level >= 1 AND escalation_level <= 5', name='valid_escalation'),
    )
...
class AddressProfile(Base, TimestampMixin, AuditMixin):
    ...
    address_type = Column(String(20), nullable=False, index=True)  # P2PKH, P2SH, P2WPKH, etc.
    ...
    __table_args__ = (
        ...
        CheckConstraint('vulnerability_score >= 0 AND vulnerability_score <= 1', name='vulnerability_range'),
        CheckConstraint('risk_score >= 0 AND risk_score <= 1', name='address_risk_range'),
...
Index('idx_transactions_compound', TransactionRecord.network, TransactionRecord.is_legacy, TransactionRecord.seen_at)
Index('idx_signatures_compound', QuantumSignature.confidence_score, QuantumSignature.threat_level, QuantumSignature.created_at)
Index('idx_alerts_compound', ThreatAlert.threat_level, ThreatAlert.response_status, ThreatAlert.created_at)
Index('idx_events_compound', SecurityEvent.event_type, SecurityEvent.severity, SecurityEvent.created_at)
