...
if (!globalThis.ReadableStream) {
  // `stream/web` got introduced in v16.5.0 as experimental
...
try {
  // Don't use  prefix for this, require+node: is not supported until node v14.14
...
class Body {
	...
	async formData() {
		...
		const {toFormData} = await Promise.resolve().then(function () { return require('./multipart-parser-61109072.cjs'); });
...
const { stat, mkdtemp } = node_fs.promises;
...
process__default.default.once('exit', () => {
});
